...
for review in reviews:
  ...
  #ger_attribute('innerHTML') : Html요소를 text형태로 가져온다
  soup = BeautifulSoup(review.ger_attribute('innerHTML'), 'html.parser')
  ...
  #리뷰 날짜 찾기
  date = soup.find(class_='p2Tk0b').text
  ...
  #scv 파일로 데이터프레임 저장
  filename = datetime.now().strfrime('result/%Y-%m-%d_%H-%M-%S.csv')
  #df.to_csv() csv파일로 저장
